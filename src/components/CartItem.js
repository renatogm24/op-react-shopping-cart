...
function CartItem({ product, updateQuantity }) {
  const { category, id, name, price, image, quantity, color, size, colors } =
    product;
  ...
  const quantityChangeHandler = (e) => {
    ...
    updateQuantity(id, +value, color, size);
  };

  const incrementQuantity = () => {
    inputEl.current.value = quantity + 1;
    updateQuantity(id, +inputEl.current.value, color, size);
  };
      ...
      >
        <div className={styles.cartContent}>
          <img
            ...
            src={colors.filter((element) => element.name === color)[0].image}
            alt={name}
          />
